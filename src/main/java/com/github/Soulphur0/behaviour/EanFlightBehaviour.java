...
public class EanFlightBehaviour {
    ...
    private static Vec3d ean_calcFlightMovementVector(FlightConfig configInstance, LivingEntity player,
        ...
        double altitudeCalculatedSpeed = (configInstance.isAltitudeDeterminesSpeed()) ? MathHelper.clamp(
                EanMath.getLinealValue(curveStart, minSpeed, curveEnd, maxSpeed, playerAltitude), minSpeed,
                maxSpeed) : minSpeed;
        ...
        movementVector = player.getVelocity()
                .add(0.0, fallSpeedConstant * (-1.0 + (double) fallSpeedMultiplier * 0.75), 0.0);
        ...
        if (movementVector.y < 0.0 && angleToTheGround > 0.0) {
            ...
            // % Set Y=0.0 to turn off downwards speed.
            movementVector = movementVector.add(rotationVector.x * horizontalSpeedValue / angleToTheGround,
                    verticalSpeedValue, rotationVector.z * horizontalSpeedValue / angleToTheGround);
        }
        ...
        if (pitchInRadians < 0.0f && angleToTheGround > 0.0) {
            ...
            movementVector = movementVector.add(-rotationVector.x * verticalSpeedValue / angleToTheGround,
                    Math.min((verticalSpeedValue * 3.2), 0.1D),
                    -rotationVector.z * verticalSpeedValue / angleToTheGround);
        }
